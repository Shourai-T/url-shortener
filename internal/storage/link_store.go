package storage

import (
	"context"
	"errors"
	"fmt"
	"time"

	"github.com/Shourai-T/url-shortener/internal/model"
	"github.com/Shourai-T/url-shortener/internal/utils"
	"github.com/jackc/pgx/v5"
	"github.com/jackc/pgx/v5/pgxpool"
)

type Store struct {
	db    *pgxpool.Pool
	redis *RedisClient
}

func NewStore(db *pgxpool.Pool, redis *RedisClient) *Store {
	return &Store{db: db, redis: redis}
}

// CreateLink thực hiện sinh mã và lưu vào DB với thời gian hết hạn mặc định 30 ngày
func (s *Store) CreateLink(originalURL string) (*model.Link, error) {
	var link model.Link
	link.OriginalURL = originalURL
	// Mặc định hết hạn sau 30 ngày
	link.ExpiresAt = time.Now().Add(30 * 24 * time.Hour)

	ctx := context.Background()

	// Thử tối đa 3 lần nếu bị trùng mã code (Collision handling)
	for i := 0; i < 3; i++ {
		link.ShortCode = utils.GenerateRandomString(6) // Sinh mã 6 ký tự

		query := `INSERT INTO links (original_url, short_code, expires_at) 
                  VALUES ($1, $2, $3) RETURNING id`

		// Sử dụng pgxpool
		err := s.db.QueryRow(ctx, query, link.OriginalURL, link.ShortCode, link.ExpiresAt).Scan(&link.ID)
		if err == nil {
			// Thành công, cache ngay vào Redis để đọc nhanh
			// Set TTL bằng thời gian còn lại của link
			ttl := time.Until(link.ExpiresAt)
			if ttl > 0 {
				_ = s.redis.SetOriginalURL(ctx, link.ShortCode, link.OriginalURL, ttl)
			}
			return &link, nil
		}

		// Nếu lỗi không phải do trùng lặp thì return lỗi luôn
		fmt.Printf("Retry generating code due to error: %v\n", err)
	}

	return nil, fmt.Errorf("failed to generate unique code after retries")
}

// GetAndIncrement lấy URL gốc và tăng lượt click (Async via Redis)
func (s *Store) GetAndIncrement(shortCode string) (string, error) {
	ctx := context.Background()

	// 1. Kiểm tra Cache Redis trước (Read Path)
	// Redis tự động expire key theo TTL mà ta set ở CreateLink
	cachedURL, err := s.redis.GetOriginalURL(ctx, shortCode)
	if err == nil && cachedURL != "" {
		// Cache Hit
		// 2. Async Write: Tăng click trong Redis, không ghi DB ngay
		go func() {
			_ = s.redis.IncrementClick(context.Background(), shortCode)
		}()
		return cachedURL, nil
	}

	// 3. Cache Miss: Đọc từ DB (cần check thêm expires_at)
	query := `SELECT original_url, expires_at FROM links WHERE short_code = $1`
	var originalURL string
	var expiresAt time.Time

	err = s.db.QueryRow(ctx, query, shortCode).Scan(&originalURL, &expiresAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return "", fmt.Errorf("link not found")
		}
		return "", fmt.Errorf("failed to get link: %w", err)
	}

	// Kiểm tra hết hạn ở DB (để chắc chắn)
	if time.Now().After(expiresAt) {
		return "", fmt.Errorf("link expired")
	}

	// 4. Update Cache lại cho lần sau
	// Tính lại TTL
	ttl := time.Until(expiresAt)
	if ttl > 0 {
		_ = s.redis.SetOriginalURL(ctx, shortCode, originalURL, ttl)
	}

	// 5. Tăng click (Async)
	go func() {
		_ = s.redis.IncrementClick(context.Background(), shortCode)
	}()

	return originalURL, nil
}

// GetLinkStats để xem thông tin link
func (s *Store) GetLinkStats(shortCode string) (*model.Link, error) {
	var link model.Link
	ctx := context.Background()
	query := `SELECT original_url, short_code, click_count, created_at, expires_at FROM links WHERE short_code = $1`
	err := s.db.QueryRow(ctx, query, shortCode).Scan(&link.OriginalURL, &link.ShortCode, &link.ClickCount, &link.CreatedAt, &link.ExpiresAt)
	if err != nil {
		return nil, err
	}
	return &link, nil
}

// GetAllLinks lấy danh sách link có phân trang
func (s *Store) GetAllLinks(limit, offset int) ([]model.Link, error) {
	// Lấy thêm cột expires_at
	query := `SELECT original_url, short_code, click_count, created_at, expires_at 
	          FROM links 
	          ORDER BY created_at DESC 
	          LIMIT $1 OFFSET $2`

	ctx := context.Background()
	rows, err := s.db.Query(ctx, query, limit, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

	var links []model.Link
	for rows.Next() {
		var link model.Link
		if err := rows.Scan(&link.OriginalURL, &link.ShortCode, &link.ClickCount, &link.CreatedAt, &link.ExpiresAt); err != nil {
			return nil, err
		}
		links = append(links, link)
	}

	if err := rows.Err(); err != nil {
		return nil, err
	}

	return links, nil
}

// DeleteLink xóa link khỏi DB và Redis
func (s *Store) DeleteLink(shortCode string) error {
	ctx := context.Background()

	// 1. Xóa khỏi DB
	query := `DELETE FROM links WHERE short_code = $1`
	tag, err := s.db.Exec(ctx, query, shortCode)
	if err != nil {
		return fmt.Errorf("failed to delete from db: %w", err)
	}

	// Kiểm tra xem có row nào bị xóa không
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("link not found")
	}

	// 2. Xóa khỏi Redis (Cache & Click Count)
	// Xóa key URL cache
	_ = s.redis.DeleteKey(ctx, "url:"+shortCode)
	// Xóa key Click cache (nếu có)
	_ = s.redis.DeleteKey(ctx, "click:"+shortCode)

	return nil
}
